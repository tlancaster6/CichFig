...
class HMMAnalyzer:
    ...
    def retDBScanMatrix(self, minMagnitude=0, densityFilter=1):
        ...
        threshold = counts[
                        0] * densityFilter / 100  # This excludes frames where too many pixels are changing in a frame (i.e. lighting changes)
    ...
    def retImage(self, t):
        ...
        if self.t == t:
            ...
            self.cached_frame = self.data[(self.data[:, 0] <= t) & (self.data[:, 1] >= t)][:, 2].reshape(
    ...
    def magDensity(self, frame, x0=20, t0=60):
        ...
        diffs = abs(self.retImage(frame).astype(int) - self.retImage(max(frame - self.frameblock, 0)).astype(int))
        dens = self.retDifference(max(frame - t0, 0), min(frame + t0, self.frames))
